...
if (isset($_GET['id']) && is_numeric($_GET['id'])) {
	$id = intval($_GET['id']);
	// Debug: Print the ID
	echo "Vacancy ID: $id<br>";
	
	$qry = $conn->query("SELECT * FROM vacancy WHERE id = $id");
	if ($qry && $qry->num_rows > 0) {
		$row = $qry->fetch_assoc();
		extract($row);
	} else {
		// Handle case where no vacancy is found
		echo "No vacancy found with the given ID.";
		exit;
	}
} else {
	// Handle case where no ID is provided
	echo "No vacancy ID provided. Please make sure you're using a valid link.";
	exit;
}
?>
